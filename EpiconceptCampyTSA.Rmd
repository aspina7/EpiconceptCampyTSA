...
We will mainly use packages which come with R (base code), but where it makes things easier we will use add-on packages. All the R packages you need for the exercises can be installed over the Internet.

Throughout the case study, we will be using the following packages:
...
Please install them before starting. You can also install packages using the *install.packages* function, as below.  
...
Here too, you can load multiple packages at the same time within a loop. 
...
## Variable types
Campy is now a *R* object, of the data frame type. A data frame is simply how datasets are classified in *R*, thus given class "data.frame". You can check the type of object by typing: class(campy)
...
# (relying on default format YYYY-MM-DD defined as “%Y-%m-%d” )   
...
You then need to use the *aggregate* function to collapse your dataset and get counts of Campylobacter cases by day of the week. 


```{r}
# Using campy dataframe 
  # sum counts by day of the week 
campy_dayweek <- aggregate(count ~ dayweek, campy, FUN = sum)
```
...
Let us check for numeric attraction by day of the month (think of potential higher notification on the 1^st^ of the month for administrative purposes). 
As we have seen that there are much fewer notifications during weekends, let us exclude Saturday and Sunday from this analysis. We create a new object that is the copy of ‘campy’ but without all weekend records. 


```{r}
# Keep rows from campy that are not saturday or sunday
campy_noweekend <- subset(campy, 
...
campy_noweekend$daynum <- format(campy_noweekend$time, "%d")

# Check the class of the new variable
class(campy_noweekend$daynum)

# Change to numeric 
campy_noweekend$daynum <- as.numeric(campy_noweekend$daynum)
```


We then need to aggregate the number of cases by day of the month, by creating a new data frame "campy_noweekend_daynum",

```{r}
# Using campy_noweekend dataframe
  # sum counts by day
campy_daynum <- aggregate(count ~ daynum, campy_noweekend, FUN = sum)
```
...
# Using campy dataframe
  # sum counts by week
campy_weeknum <- aggregate(count ~ week, campy, FUN = sum)
```

We will then create a Time-Series object. These are vector or matrices with class of "ts" (and additional attributes) which represent data that have been sampled at equispaced points in time. 
Working with a Time-Series object will make it easier to analyse the time series. 
We only keep the counts variable and then define the time unit of the new object by specifying the starting point (beginning of first week of 2007) and the frequency (52 weeks). 

```{r} 
# Create time series only using campy_weeknum
campy_ts <- ts(campy_weeknum$count, start = c(2007, 1), frequency = 52)
```
...
Kurtosis is a measure of whether the data are heavy-tailed or light-tailed relative to a normal distribution. The kurtosis for a standard normal distribution is three. 
...
We first need to compute a rolling standard deviation deviation using the zoo package (Z’s Ordered Observations). In this case we choose a moving window of 10 weeks. This means that for each time point a standard deviation is calculated based on the five points before and after. 
...
There appears to be a significant difference in variance over time. 

According to our results, the time series is not stationary which is expected with surveillance data. Indeed, we do not expect our dataset to be stationary at this point.
...
acplot <-  acf(campy_ts, lag.max = 52, plot = FALSE)
```
...
plot(acplot, ci.type = "ma", main = "Autocorrelation", xlab = "Lag (week)")
```

The first line on this autocorrelation graph shows the correlation between each observation and itself (at lag zero, therefore always 1), the second shows correlation between each observation and the preceding one (lag one), and so on. 
Blue lines show the 95% confidence intervals. 
Where lines go above or below this there is significant autocorrelation (in order to produce a similar confidence intervals to Stata, add *ci.type = "ma"* for *"moving average process"* to the acf function – where confidence limits assume for lag k an MA(k-1) input instead of white noise). 
...
```{r}
# Plot your time series with annual moving average
...
We will check for seasonality on the data after the trend is removed.
We can find the most important periods in the data by doing a spectral analysis. 
This can be done using the *spec.pgram* function and the argument *detrend = TRUE* (default value). By specifying *fast* as false, it stops the function from "padding" the series with extra zeros to enable fast fourier transform. 


```{r}

# Get spectrograme with detrended time series
periodo <- spec.pgram(campy_ts, fast = FALSE, plot = FALSE)
...
1 / periodo$freq[order(-log(periodo$spec))] * 52
...
In this case we go back to linreg3 which contained periods of 52.25 and 17.4 weeks. 
...
It would also be possible to investigate that the variance is constant over time. 
We could use a 5-week interval for this. 
...
# Linear regression
residreg_var <- lm(varianceresid ~ t)

# View linear outcome 
summary(residreg_var)

```

There does not apper to be a significant difference in variance over time.

Our results suggest that the residuals are stationary.
...
```{r}
# Estimate autocorrelation without plotting
acplotresid <-  acf(linreg3$residual, plot = FALSE)
plot(acplotresid, ci.type = "ma")
```
...
In fact, we will see that the normality of residuals is not improved in the model with 3 seasonal terms. The data remain stationary (no increasing variance with increasing mean). However, the ACF plot is improved. There is still autocorrelation among the first few lags, but it is lower and we see less seasonal pattern remaining in the residuals. 

<!---CHUNK_PAGEBREAK---> 

# Question 6 

Using the model containing a linear trend and two seasonal periods at 52.25 and 17.4 weeks, model the data including 95% prediction interval and graph them. 
...
campy_sts <- sts(observed = campy_weeknum$count,
                 start = c(2007, 1), 
                 freq = 52)
```
...
However, in order to identify True Positive (TP), False Negative (FN), True Negative (TN) and False Negative (FN), we would need to provide each algorithm with a *"Gold Standard"* of true Campylobacter outbreaks over the study period.
